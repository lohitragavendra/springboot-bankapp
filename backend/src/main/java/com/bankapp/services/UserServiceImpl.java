...
public class UserServiceImpl implements UserService {
    ...
    public java.util.List<TransactionDTO> getAllTransactionsForUser(String email) {
        ...
        public UserDTO updateProfile(String email, UserDTO userDTO) {
                ...
                if (userDTO.getDarkMode() != null)
                        user.setDarkMode(userDTO.getDarkMode());
                userRepository.save(user);
                return getProfile(email);
        }

        @Override
        public BankResponse changePassword(String email, ChangePasswordRequest request) {
                Optional<User> userOpt = userRepository.findByEmail(email);
                if (userOpt.isEmpty()) {
                        return BankResponse.builder()
                                .responseCode("01")
